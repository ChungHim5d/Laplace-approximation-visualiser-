    ...
    <div className="flex flex-col gap-5 w-full">
        ...
        <div className="flex items-center justify-center gap-6">
            <div className="flex flex-col items-center">
                <span className="text-xs text-slate-400 uppercase tracking-wider mb-1">Hessian Matrix (1x1)</span>
                <div className={`w-24 h-24 flex items-center justify-center rounded-lg border-2 shadow-sm transition-colors duration-500 ${colorClass}`}>
                    <span className="font-mono font-bold text-3xl">{displayHessian.toFixed(1)}</span>
                </div>
            </div>

            <div className="text-base text-slate-600 max-w-[160px] leading-tight">
                {isConcaveDown 
        ...
        {/* Curvature Dial */}
        <div className="relative h-44 bg-slate-50 rounded-xl border border-slate-100 flex items-end justify-center pb-6 overflow-hidden mt-2">
            <div className="absolute top-3 left-4 flex items-center gap-2 text-slate-400">
                <Gauge size={18} />
                <span className="text-sm uppercase font-bold">Curvature Dial</span>
            </div>
            ...
            {/* Needle */}
            <div 
                className="absolute bottom-6 left-1/2 w-1.5 h-32 bg-slate-800 origin-bottom rounded-full transition-transform duration-100 z-20"
                style={{ 
                    transform: `translateX(-50%) rotate(${rotation}deg)`,
                    transformOrigin: 'bottom center'
                }}
            >
                <div className="w-4 h-4 bg-slate-800 rounded-full absolute bottom-0 left-1/2 -translate-x-1/2"></div>
            </div>

            {/* Labels */}
            <div className="absolute bottom-3 left-12 text-sm font-bold text-emerald-700">Sharp Peak</div>
            <div className="absolute bottom-3 right-12 text-sm font-bold text-rose-700">Valley</div>
        </div>
